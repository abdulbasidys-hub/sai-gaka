...
export default function AddTransactionSheet({ open, onClose, prefill }) {
  const { addTransaction, BUDGET_CATEGORIES, ALL_CATEGORIES, balanceGBP, balanceNGN, budgets, totalIncomeGBP, totalIncomeNGN, salarySettings } = useFinance();
  ...
  // Reset on close, or apply prefill
  useEffect(() => {
    if (open) {
      if (prefill) {
        setType('expense');
  ...
  // Has the user ever put money in? Only block if they have income set up
  const hasIncome = account === 'GBP'
    ? (totalIncomeGBP > 0 || salarySettings?.amountGBP > 0)
    : (totalIncomeNGN > 0 || salarySettings?.amountNGN > 0);

  const wouldOverdraw = type === 'expense' && amount && parseFloat(amount) > currentBalance && hasIncome;
  const lowBalance = type === 'expense' && amount && parseFloat(amount) > currentBalance && !hasIncome;

  const selectedSource = INCOME_SOURCES.find(s => s.id === incomeSource);

  const isSubmitReady = amount && !isNaN(Number(amount)) && Number(amount) > 0
    && !wouldOverdraw
    && !loading
              ...
              <div style={{ padding: '12px 16px 0 16px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                  <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '18px', fontWeight: '800', color: 'var(--text-primary)' }}>
                    {prefill ? 'Confirm Payment' : 'Add Transaction'}
                  </h3>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button onClick={onClose} style={{ display: 'flex', alignItems: 'center', gap: '2px', color: 'var(--text-muted)', fontSize: '12px', fontWeight: '600' }}>
                      <ChevronDown size={18} />
                    </button>
                  </div>
                </div>

                {/* Type toggle — hidden when prefilled */}
                {!prefill && (
                  <div style={{ display: 'flex', background: 'var(--bg-elevated)', borderRadius: '10px', padding: '3px', marginBottom: '14px' }}>
                    {['expense', 'income'].map(t => (
                      <button key={t} onClick={() => setType(t)} style={{
                        ...
                        {/* Out = arrow up (money leaving), In = arrow down (money coming in) */}
                ...
                <div style={{
                  ...
                  marginBottom: (wouldOverdraw || lowBalance) ? '4px' : '14px',
                }}>
                ...
                {lowBalance && (
                  <p style={{ fontSize: '12px', color: 'var(--accent-amber)', fontWeight: '600', marginBottom: '10px' }}>
                    ⚠️ No income set yet — set up salary or add income first
                  </p>
                )}

                {/* EXPENSE: category + sub-items */}
                {type === 'expense' && (
                  <>
                    ...
                    {incomeSource === 'borrowed' && (
                      <div style={{ background: 'var(--accent-red-dim)', border: '1px solid var(--accent-red)', borderRadius: 'var(--radius-md)', padding: '10px 12px', marginBottom: '12px' }}>
                        <p style={{ fontSize: '12px', color: 'var(--accent-red)', fontWeight: '600' }}>
                          🤝 Tracked as borrowed. Mark as repaid later in Transactions.
                        </p>
                      </div>
                    )}
                  </>
                ...
                {/* Submit button — always visible, clearly enabled/disabled */}
                <motion.button
                  onClick={handleSubmit}
                  disabled={!isSubmitReady}
                  whileTap={isSubmitReady ? { scale: 0.97 } : {}}
                  style={{
                    width: '100%', padding: '15px', borderRadius: 'var(--radius-md)',
                    background: !isSubmitReady
                      ? 'var(--bg-elevated)'
                      : type === 'expense'
                    ...
                    color: !isSubmitReady ? 'var(--text-muted)' : '#fff',
                    fontSize: '15px', fontWeight: '800', fontFamily: 'var(--font-display)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px',
                    cursor: isSubmitReady ? 'pointer' : 'default',
                  }}
                >
                  <Check size={16} />
                  {loading ? 'Saving…'
                    : wouldOverdraw ? 'Insufficient balance'
                    : `Save ${account} ${type === 'expense' ? 'Expense' : 'Income'}`}
                </motion.button>

                {/* Safe-area spacer — ensures button is never hidden by home bar */}
